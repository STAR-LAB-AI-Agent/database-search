...
- 基于本地资料**提问**：先用 search 检索片段，再依据片段作答（本 Skill 不内置 LLM，由智能体完成生成）
- 管理资料库：导入、列出、删除、统计
...
- 文件新增/删除后，`search` 会自动同步；也可用 `ingest <路径>` 显式刷新；`list`/`stats` 显示的是当前索引。
...
3. 用户：「根据本地资料，什么是检索增强生成？」
   → 先 `python cli.py search "检索增强生成" --top-k 5`，再依据返回片段作答并注明来源
